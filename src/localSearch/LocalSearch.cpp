...
bool LocalSearch::bestImprovementOrOpt(const Data &data, Solution &s, const int blockSize)
{
    ...
    for(int i = 1; i < s.sequence.size() - blockSize - 1; ++i)
    {
        ...
        {
            if (j >= i - 1 && j <= i + blockSize - 1) continue; // não faz sentido reinserir o i entre [i-1, i] e nem entre [i, i + blockSize + 1]
            if (j == i - 1 && blockSize == 1) continue; //evita swap se blockSize = 1
